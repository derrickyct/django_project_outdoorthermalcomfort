...
from .variables import filter_option, FIRST_CHOICES, SECOND_CHOICES, THIRD_CHOICES,GENDER_CHOICES, AGEGRP_CHOICES, \
    LOCATION_CHOICES, CLIMATE_CHOICES, TIME_CHOICES, SEASON_CHOICES, CITY_CHOICES, SVF_CHOICES, age_min, age_max, \
    height_min, height_max, weight_max, weight_min, metabolic_rate_min, metabolic_rate_max, clothing_index_min, \
    clothing_index_max, thermal_history_min, thermal_history_max, air_temp_min, air_temp_max, relative_humidity_min, \
    relative_humidity_max, wind_speed_min, wind_speed_max, global_temp_min, global_temp_max, mean_radiant_temp_min, \
    mean_radiant_temp_max, radiation_min, radiation_max, pet_min, pet_max, utci_min, utci_max
...
def DataFilter(t, request):
    ...
    if isValid(metabolic_rate_query) and metabolic_rate_query.split(',') != [f'{metabolic_rate_min:g}',
                                                                             f'{metabolic_rate_max:g}']:
        print('metabolic rate')
        isSelected = True
        t = t.filter(metabolic_rate__range=metabolic_rate_query.split(','))
    if isValid(clothing_index_query) and clothing_index_query.split(',') != [f'{clothing_index_min:g}',
                                                                             f'{clothing_index_max:g}']:
        print('clothing index')
        isSelected = True
        t = t.filter(clothing_index__range=clothing_index_query.split(','))
    if isValid(thermal_history_query) and thermal_history_query.split(',') != [f'{thermal_history_min:g}',
                                                                               f'{thermal_history_max:g}']:
    ...
    if isValid(relative_humidity_query) and relative_humidity_query.split(',') != [f'{relative_humidity_min:g}',
                                                                                   f'{relative_humidity_max:g}']:
    ...
    if isValid(mean_radiant_temp_query) and mean_radiant_temp_query.split(',') != [f'{mean_radiant_temp_min:g}',
                                                                                   f'{mean_radiant_temp_max:g}']:
